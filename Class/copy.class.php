...
function rsDados($id='', $orderBy='', $limite='', $pagina_individual='', $ativo='') {
  ...
  try {
    $sql = "SELECT * FROM tbl_copy where 1=1 $sql $sqlOrdem $sqlLimite";
...
function add($redireciona='') {
    if(isset($_POST['acao']) && $_POST['acao'] == 'addCopy') {
...
function editar($redireciona='copy.php') {
    if(isset($_POST['acao']) && $_POST['acao'] == 'editaCopy') {
if(isset($_POST['meta_title']) && !empty($_POST['meta_title'])){
    $meta_title = $_POST['meta_title'];
}else{
    $meta_title = '';
...
if($pagina_individual == 'S') {
  echo "  <script>
          ...
          window.location='editar-copy.php?pi=S&id=$id';
      </script>";
  exit;
} else {
  echo "  <script>
...
function excluir() {
  if (isset($_GET['acao']) && $_GET['acao'] == 'excluirServico') {
    try {
      $sql = "DELETE FROM tbl_copy WHERE id=? ";
      $stm = $this->pdo->prepare($sql);
