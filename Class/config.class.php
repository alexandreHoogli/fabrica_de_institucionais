	...
	class ConfigSistema {
		...
		function rsDados() {
			...
			try{   
				...
				$this->favicon_lp = $rsDados[0]->favicon_lp;
				$this->logo_principal = $rsDados[0]->logo_principal;
				$this->logo_branca = $rsDados[0]->logo_branca;
				$this->logo_verde = $rsDados[0]->logo_verde;
				...
				$this->sobre_breve = $rsDados[0]->sobre_breve;
		...
		function editar() {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editarConfig') {
				...
				$sobre_breve = filter_input(INPUT_POST, 'sobre_breve');
		...
		function editarAparencia() {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editarConfigAparencia') {
				...
				try{   
					...
					$sql = "UPDATE tbl_config SET logo_principal=?, logo_rodape=?, logo_mobile=?, link_fonte_titulo=?, font_family_titulo=?, font_weight_titulo=?, link_fonte_texto_apoio=?, font_family_texto_apoio=?, font_weight_texto_apoio=?, cor_primaria=?, cor_secundaria=?, cor_titulo=?, cor_background=?, cor_botao=?, cor_botao_hover=?, cor_menu=?, favicon=?, favicon_lp=?, logo_branca=?, logo_verde=? WHERE id=? ";   
					$stm = $this->pdo->prepare($sql);  
					...
					$stm->bindValue(18, upload('favicon_lp', $pastaArquivos, 'N'));
					$stm->bindValue(19, upload('logo_branca', $pastaArquivos, 'N'));
					$stm->bindValue(20, upload('logo_verde', $pastaArquivos, 'N'));
					$stm->bindValue(21, 1);
					$stm->execute();  
