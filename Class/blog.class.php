...
$section_1_conteudo_3 = (isset($_POST['section_1_conteudo_3']) && !empty($_POST['section_1_conteudo_3'])) ? $_POST['section_1_conteudo_3'] : '' ;
 $id = filter_input(INPUT_POST, 'id');
          if (isset($_POST['pagina_individual']) && !empty($_POST['pagina_individual'])) { $pagina_individual = $_POST['pagina_individual'];  } else {   $pagina_individual = gerarTituloSEO($section_1_img_1);  } $maximo = 180000;if ($_FILES["section_1_img_1"]["size"] > $maximo) {
...
if ($_FILES["section_1_img_2"]["size"] > $maximo) {
    ...
    try { 
        if(file_exists('Connection/conexao.php')) {
            ...
            $sql = "UPDATE tbl_blog SET meta_title=?, meta_keywords=?, meta_description=?, section_1_img_1=?, section_1_image_alt_1=?, section_1_img_2=?, section_1_image_alt_2=?, section_1_conteudo_3=? WHERE id=?";        $stm = $this->pdo->prepare($sql);
